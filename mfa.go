...
func main() {
    token := "YOUR TOKEN"
    serverID := "YOUR SERVER ID(no required gerek yok oc)"
    ...
    for {
        if newToken := getMFAToken(token, serverID, password); newToken != "" {
            ...
            ioutil.WriteFile("mfa.json", jsonData, 0644)
            fmt.Println("MFA token saved")
            mfaMutex.Unlock()
        } else {
            fmt.Println("Failed to get MFA token, retrying...")
        }
